...
import { useDiscounts, type PromoCoupon } from "@/lib/discounts";
...
import {
  Plus, AlertTriangle, ShieldAlert, ShieldCheck, Trash2, CalendarIcon, Package, ScanLine,
  Receipt, Banknote, ArrowRight, Eye, EyeOff, Truck, Undo2, Tag, Check, Sparkles, X, Gift, Coins, Layers
} from "lucide-react";
...
function NewInvoicePage() {
  ...
  const { currentStaff, currentOpenShift } = useStaffAndShifts();
  const {
    coupons,
    validateCoupon,
    recordCouponUsage,
  ...
  const customerLoyalty = useMemo(() => {
    ...
    return customerLoyaltyStats.find((s) => s.customer.id === customerId);
  }, [customerId, customerLoyaltyStats]);

  const autoOffersResult = useMemo(() => {
    ...
    return calculateAutoOffers(items);
  }, [products, calculateAutoOffers]);

  const marginSafety = useMemo(() => {
    return checkMarginSafety(subtotal, totalCost, discountValue);
  }, [subtotal, totalCost, discountValue, checkMarginSafety]);
  ...
  const applyCouponCode = (code: string) => {
    const res = validateCoupon(code, subtotal, customer?.customerType, customer?.id);
    if (!res.valid) {
      setCouponError(res.errorReason || "كود الخصم غير صالح");
      toast.error(res.errorReason || "كود الخصم غير صالح");
      return;
    }
    setAppliedCoupon(res.coupon || null);
    setDiscountAmt(String(res.discountAmount));
    setDiscountPct(String(res.discountPct));
    setCouponError(null);
    setCouponCodeInput(res.coupon?.code || code.toUpperCase());
    toast.success(`تم تطبيق كود الخصم (${res.coupon?.code}): وفرت ${fmt(res.discountAmount)} ج.م ✓`);
  };

  const applyAutoOffers = () => {
    if (autoOffersResult.suggestedDiscountAmount <= 0) return;
    setAppliedCoupon(null);
    setDiscountAmt(String(autoOffersResult.suggestedDiscountAmount));
    setDiscountPct(subtotal > 0 ? ((autoOffersResult.suggestedDiscountAmount / subtotal) * 100).toFixed(1) : "0");
    toast.success(`تم تطبيق عروض الكميات والباقات: خصم ${fmt(autoOffersResult.suggestedDiscountAmount)} ج.م ✓`);
  };

  const redeemLoyaltyCouponForCurrentInvoice = () => {
    if (!customer || !customerLoyalty || !customerLoyalty.canRedeem) return;
    const cpn = generateCustomerLoyaltyVoucher(customer, customerLoyalty.availablePoints);
    applyCouponCode(cpn.code);
  };
  ...
  const reset = () => {
    ...
    setAppliedCoupon(null); setCouponCodeInput(""); setCouponError(null);
  ...
  return (
    ...
    <>
      ...
      {/* Staff & Shift Live Status Bar */}
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-2xl bg-card/60 border border-border/40 text-xs">
        <div className="flex items-center gap-2">
          <span className="w-2.5 h-2.5 rounded-full bg-success animate-pulse" />
          <span className="text-muted-foreground">الكاشير المسجل:</span>
          <strong className="text-foreground">{currentStaff.name}</strong>
          <Badge variant="outline" className="text-[10px] py-0 px-1.5">
            {currentStaff.role === "admin" ? "مدير عام" : currentStaff.role === "manager" ? "مشرف" : "كاشير"}
          </Badge>
        </div>
        ...
        <div className="flex items-center gap-3">
          {currentOpenShift ? (
            <span className="text-muted-foreground text-[11px]">
              الوردية النشطة: <strong className="text-foreground">#{currentOpenShift.shiftNumber}</strong>
            </span>
          ) : (
          ...
          <Button asChild variant="ghost" size="sm" className="h-7 px-2.5 text-xs text-primary font-bold">
            <Link to="/staff">
              إدارة الورديات
            </Link>
          </Button>
...
<Badge variant="outline" className={cn("gap-1 font-bold",
          ...
          <div className="space-y-3">
                    ...
                    >
                      <div className="grid grid-cols-1 items-end gap-2 sm:grid-cols-[minmax(0,1fr)_76px_96px_96px_32px]">
                        ...
                        <div>
                          ...
                          <Input
                            type="number"
                            value={currentStaff.permissions.canViewCostAndProfit ? p.cost : "•••"}
                            disabled={!currentStaff.permissions.canViewCostAndProfit}
                            onChange={(e) => updateProduct(p.id, { cost: e.target.value })}
                            className={cn(privacy && "privacy-blur", "h-9 bg-white/5 border-white/10 text-center font-mono text-xs")}
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            type="number"
                            value={p.price}
                            disabled={!currentStaff.permissions.canChangePrice}
                            onChange={(e) => updateProduct(p.id, { price: e.target.value })}
                            className={cn("h-9 bg-white/5 border-white/10 text-center font-mono font-bold text-xs", !currentStaff.permissions.canChangePrice && "opacity-85 cursor-not-allowed")}
                            title={!currentStaff.permissions.canChangePrice ? "تعديل السعر مقفول بصلاحيات الكاشير" : undefined}
                          />
                        </div>
            ...
            {/* الخصم والضريبة والكوبونات */}
            <div className="rounded-[1.75rem] border border-border/30 p-5 space-y-4 bg-card/40">
                <div className="flex items-center justify-between">
                  <span className={cn("text-xs font-bold font-mono", discountValue > 0 ? "text-warning" : "text-muted-foreground/50")}>
                    − {fmt(discountValue)} ج.م
                  </span>
                  <div className="flex items-center gap-1.5">
                    <Tag className="w-3.5 h-3.5 text-primary" />
                    <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">الخصومات والعروض</Label>
                  </div>
                </div>

                {/* كود وقسيمة الخصم (Promo Code) */}
                <div className="p-3.5 rounded-2xl border border-border/40 bg-muted/20 space-y-2.5">
                  ...
                  {appliedCoupon ? (
                    <div className="flex items-center justify-between p-2.5 rounded-xl bg-success/10 border border-success/30 text-xs">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-black px-2 py-0.5 rounded bg-success/20 text-success uppercase tracking-wider">
                          {appliedCoupon.code}
                        </span>
                        <span className="text-foreground font-medium">{appliedCoupon.title}</span>
                      </div>
                      <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Input
                        type="text"
                        placeholder="أدخل كود الخصم (مثال: WELCOME10)"
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            e.preventDefault();
                            if (couponCodeInput.trim()) applyCouponCode(couponCodeInput);
                          }
                        }}
                        className="font-mono uppercase text-xs h-10 rounded-xl bg-background/50"
                      />
                      <Button
                        type="button"
                        variant="outline"
                        ...
                        onClick={() => {
                          if (couponCodeInput.trim()) applyCouponCode(couponCodeInput);
                          else toast.error("يرجى كتابة كود الخصم");
                        }}
                        className="h-10 px-4 rounded-xl text-xs font-bold gap-1 shrink-0"
                      >
                        تطبيق
                      </Button>
                    </div>
                  ...
                  {/* Active available coupons quick bar */}
                  {!appliedCoupon && coupons.filter((c) => c.active).length > 0 && (
                    <div className="pt-1">
                      <div className="text-[10px] text-muted-foreground mb-1">كوبونات سارية مقترحة:</div>
                      <div className="flex flex-wrap gap-1.5">
                        {coupons.filter((c) => c.active).slice(0, 3).map((cp) => (
                          <button
                            ...
                            className="px-2 py-0.5 rounded-lg bg-background/60 hover:bg-primary/20 hover:text-primary border border-border/40 text-[10px] font-mono font-bold transition-all"
                          >
                            {cp.code} ({cp.discountType === "percentage" ? `${cp.discountValue}%` : `${cp.discountValue}ج`})
                          </button>
                        ))}
                      </div>
                    </div>
                ...
                {/* Auto Offers detected banner */}
                {autoOffersResult.appliedOffers.length > 0 && !appliedCoupon && Number(discountAmt || 0) === 0 && (
                  <div className="p-3 rounded-2xl border border-primary/30 bg-primary/5 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Sparkles className="w-4 h-4 text-primary shrink-0 animate-pulse" />
                      <div>
                        <div className="text-xs font-bold text-foreground">
                          عروض كميات وباقات مؤهلة ({fmt(autoOffersResult.suggestedDiscountAmount)} ج.م خصم)
                        </div>
                        <div className="text-[10px] text-muted-foreground">
                          {autoOffersResult.appliedOffers.map((o) => o.title).join(" + ")}
                        </div>
                      </div>
                    </div>
                    <Button
                      type="button"
                      size="sm"
                      onClick={applyAutoOffers}
                      className="h-8 px-3 rounded-xl text-xs font-bold bg-primary text-primary-foreground gap-1 shrink-0"
                    >
                      تطبيق العرض
                    </Button>
                  </div>
                ...
                {customerLoyalty && customerLoyalty.canRedeem && !appliedCoupon && (
                  <div className="p-3 rounded-2xl border border-warning/30 bg-warning/5 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <Coins className="w-4 h-4 text-warning shrink-0" />
                      <div>
                        <div className="text-xs font-bold text-foreground">
                          مكافأة ولاء للعميل ({customerLoyalty.availablePoints} نقطة = {fmt(customerLoyalty.redeemableValueEgp)} ج.م)
                        </div>
                        <div className="text-[10px] text-muted-foreground">
                          يمكنك استبدال النقاط وتطبيق خصم مباشر على هذه الفاتورة
                        </div>
                      </div>
                    </div>
                    <Button
                      ...
                      variant="outline"
                      size="sm"
                      onClick={redeemLoyaltyCouponForCurrentInvoice}
                      className="h-8 px-3 rounded-xl text-xs font-bold text-warning hover:bg-warning/10 border-warning/30 gap-1 shrink-0"
                    >
                ...
                {discountValue > 0 && (
                  <div
                    className={cn(
                      "p-2.5 rounded-xl border text-xs font-medium flex items-center gap-2",
                      marginSafety.isSafe
                  ...
                  >
                    {marginSafety.isSafe ? (
                      <>
                        ...
                        <span>الخصم آمن ويحقق نسبة ربح {marginSafety.profitMarginPct.toFixed(1)}% (صافي ربح {fmt(marginSafety.profit)} ج.م)</span>
                      </>
                    ) : (
                      <>
                        <ShieldAlert className="w-4 h-4 text-danger shrink-0" />
                        <span>{marginSafety.warning}</span>
                      </>
                    )}
                  </div>
                )}

                {/* الخصم اليدوي المباشر */}
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-muted-foreground">أو خصم يدوي مباشر على الفاتورة:</span>
                    {currentStaff.role === "cashier" && (
                      <span className="text-[10px] text-muted-foreground">
                        أقصى خصم مسموح للكاشير: <strong className="font-mono text-primary">{currentStaff.permissions.maxDiscountPct}%</strong>
                      </span>
                    )}
                  </div>
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        type="number" min="0" max={currentStaff.permissions.maxDiscountPct || 100} placeholder="0"
                        value={discountPct}
                        onChange={(e) => {
                          const v = e.target.value;
                          const rawPct = Number(v || 0);
                          const maxPct = currentStaff.permissions.maxDiscountPct || 100;
                          if (rawPct > maxPct) {
                            toast.error(`صلاحيات الكاشير لا تسمح بتجاوز خصم ${maxPct}%`);
                            setDiscountPct(String(maxPct));
                            setAppliedCoupon(null);
                            setDiscountAmt(String(Math.round((subtotal * maxPct) / 100)));
                            return;
                          }
                          setDiscountPct(v);
                          setAppliedCoupon(null);
                        ...
                        className="h-10 rounded-xl border-border/40 bg-background/40 text-center font-mono"
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const v = e.target.value;
                          const rawAmt = Number(v || 0);
                          const maxAmt = (subtotal * (currentStaff.permissions.maxDiscountPct || 100)) / 100;
                          if (rawAmt > maxAmt && subtotal > 0) {
                            toast.error(`الخصم يتجاوز الحد المسموح للكاشير (${currentStaff.permissions.maxDiscountPct}%)`);
                            setDiscountAmt(String(Math.round(maxAmt)));
                            setAppliedCoupon(null);
                            setDiscountPct(String(currentStaff.permissions.maxDiscountPct));
                            return;
                          }
                          setDiscountAmt(v);
                          setAppliedCoupon(null);
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {[5, 10, 15, 20].map((n) => (
                      <button
                        ...
                        className="rounded-lg bg-muted/50 px-2.5 py-1 text-[11px] font-mono font-bold text-muted-foreground hover:bg-primary/20 hover:text-primary transition-all active:scale-95"
                      >
                        {n}%
                      </button>
                    ))}
                    {[50, 100].map((amt) => (
                      <button
                        ...
                        onClick={() => {
                          setAppliedCoupon(null);
                          setDiscountAmt(String(amt));
                          setDiscountPct(subtotal > 0 ? ((amt / subtotal) * 100).toFixed(1) : "0");
                        }}
                        className="rounded-lg bg-muted/50 px-2.5 py-1 text-[11px] font-mono font-bold text-muted-foreground hover:bg-primary/20 hover:text-primary transition-all active:scale-95"
                      >
                        {amt} ج.م
                    ...
                    <button
                      type="button"
                      onClick={() => { setDiscountPct(""); setDiscountAmt(""); setAppliedCoupon(null); }}
                      className="rounded-lg bg-muted/50 px-2.5 py-1 text-[11px] font-bold text-muted-foreground hover:bg-danger/10 hover:text-danger transition-all active:scale-95"
                    >
                      مسح
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 gap-3 border-t border-border/30 pt-3 sm:grid-cols-2">
                  <div className="space-y-1.5">
