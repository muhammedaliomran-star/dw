        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
          <Reveal className="h-full" delay={0}>
            <MetricCard
              ...
              sub={`${data.customers.length} عميل · ${data.invoices.length} فاتورة`}
            />
          </Reveal>
          ...
          <Reveal className="h-full" delay={210}>
            <MetricCard
              ...
              sub={incompleteCostCount > 0 ? `${incompleteCostCount} فاتورة تكلفتها غير مكتملة` : `أرباح ${m(fmt(monthGrossProfit))} − مصروفات ${m(fmt(monthExpenses))}`}
            />
          </Reveal>
