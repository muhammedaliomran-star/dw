...
function MetricCard({ label, value, icon, color, privacy, isCount, glow, sub }: { label: string, value: number, icon: React.ReactNode, color: string, privacy: boolean, isCount?: boolean, glow?: boolean, sub?: string }) {
  const blurCls = privacy ? "privacy-blur" : "privacy-clear";
  const tone = color === "success" ? "success" : color === "danger" ? "danger" : color === "primary" ? "primary" : color === "warning" ? "warning" : "neutral";
  
  const borderCls = tone === "danger"
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-2xl border bg-card/70 plate p-5 transition-[transform,background-color,border-color,box-shadow] duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-0.5", borderCls)}>
      ...
      <div className="relative">
        ...
        <div className={cn("text-2xl lg:text-3xl font-extrabold mt-4 tabular-nums text-right", colorCls, blurCls)}>
          {isCount ? <CountUp value={value} duration={1000} /> : <><CountUp value={value} duration={1000} format={(n: number) => fmt(n)} /> <span className="text-xs font-bold text-muted-foreground">ج.م</span></>}
        </div>
