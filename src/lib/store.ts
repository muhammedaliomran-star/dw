...
export const db = {
  ...
  async addCustomer(c: Omit<Customer, "id" | "createdAt">) {
    ...
    recordAuditLog({
      ...
      details: `تم إضافة العميل: ${c.name} - هاتف: ${c.phone}`,
    });
    await fetchAll();
  },
  async updateCustomer(id: string, patch: Partial<Customer>) {
    ...
    recordAuditLog({
      ...
      details: `تحديث بيانات العميل #${id.slice(0, 6)}`,
    });
    await fetchAll();
  },
  async removeCustomer(id: string) {
    ...
    recordAuditLog({
      ...
      severity: "danger",
      entityId: id,
      title: "حذف عميل من النظام",
      details: `تم حذف العميل #${id.slice(0, 6)} بشكل نهائي`,
    });
    await fetchAll();
  },
  async addInvoice(inv: Omit<Invoice, "id" | "createdAt" | "paid"> & { paid?: number; items?: Array<{ name: string; cost: number; price: number; quantity?: number }> }) {
    ...
    recordAuditLog({
      ...
      details: `إصدار فاتورة بإجمالي ${inv.total} ج.م (مقدم: ${inv.downPayment} ج.م) - أصناف: ${inv.items?.length ?? 0}`,
    });
    await fetchAll();
  },
  ...
  async removeInvoice(id: string) {
    ...
    recordAuditLog({
      ...
      severity: "critical",
      entityId: id,
      title: "حذف فاتورة بيع نهائياً",
      details: `تم حذف الفاتورة #${id.slice(0, 6)} وإعادة الأصناف المباعة إلى رصيد المخزن`,
    });
    await fetchAll();
  },
  ...
  async updateInvoice(id: string, patch: Partial<Pick<Invoice, "total" | "downPayment" | "monthlyInstallment" | "firstDueDate" | "notes">>) {
    ...
    recordAuditLog({
      ...
      details: `تعديل بيانات الفاتورة #${id.slice(0, 6)}`,
    });
    await fetchAll();
  },
  ...
  async removePayment(id: string) {
    ...
    recordAuditLog({
      ...
      severity: "danger",
      entityId: id,
      title: "حذف دفعة أو قسط محصل",
      details: `تم حذف الدفعة #${id.slice(0, 6)} من سجل الحسابات`,
    });
    await fetchAll();
  },
  async recordPayment(invoiceId: string, amount: number) {
    ...
    recordAuditLog({
      ...
      details: `تحصيل مبلغ ${amount} ج.م للفاتورة #${invoiceId.slice(0, 6)}`,
    });
    await fetchAll();
  },
  async addExpense(exp: Omit<Expense, "id" | "createdAt">) {
    ...
    recordAuditLog({
      ...
      details: `تسجيل مصروف بقيمة ${exp.amount} ج.م (بند: ${exp.category})`,
    });
    await fetchAll();
  },
  ...
  async upsertStockDeltas(items: Array<{ name: string; quantity: number; unitCost: number; barcode?: string | null }>) {
    ...
    if (items.length > 0) {
      recordAuditLog({
        ...
        module: "inventory",
        severity: "warning",
        title: "تحديث كميات المخزن والجرد",
        details: `تسوية كميات لـ ${items.length} صنف/منتج في المخزون`,
      });
    }
  },
...
export async function saveShopSettings(patch: ShopSettings) {
  ...
  recordAuditLog({
    ...
    details: `تم تحديث بيانات المحل، القوالب، أو السياسات المالية`,
  });
